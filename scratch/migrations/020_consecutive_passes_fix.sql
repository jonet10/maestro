-- Migration: 020_consecutive_passes_fix.sql
-- Description: Add consecutive_passes to game_state to resolve softlocks in PASS sequences, and fix nextRoundStarter logic to designate the actual round winner.
    ...
    IF creator_score >= target_score OR opponent_score >= target_score THEN
      ...
      BEGIN
        ...
        ELSE
          ...
          g_state := g_state || jsonb_build_object(
            'matchStatus', 'ongoing',
            'next_round_start_at', (EXTRACT(EPOCH FROM NOW() + INTERVAL '10 seconds') * 1000)::bigint, -- 10s delay in ms
