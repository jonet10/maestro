...
final class ViewingConditions {
    ...
    public static ViewingConditions make(@NonNull float[] fArr, float f, float f10, float f11, boolean z9) {
        ...
        float lerp = ((double) f20) >= 0.9d ? CamUtils.lerp(0.59f, 0.69f, (f20 - 0.9f) * 10.0f) : CamUtils.lerp(0.525f, 0.59f, (f20 - 0.8f) * 10.0f);
        float exp = z9 ? 1.0f : (1.0f - (((float) Math.exp(((-f) - 42.0f) / 92.0f)) * 0.2777778f)) * f20;
        double d10 = exp;
        if (d10 > 1.0d) {
            exp = 1.0f;
        } else if (d10 < 0.0d) {
        ...
        float f23 = 1.0f - f22;
        float cbrt = (0.1f * f23 * f23 * ((float) Math.cbrt(f * 5.0d))) + (f22 * f);
        float yFromLStar = CamUtils.yFromLStar(f10) / fArr[1];
        ...
        return new ViewingConditions(yFromLStar, ((fArr8[2] * 0.05f) + (fArr8[0] * 2.0f) + fArr8[1]) * pow, pow, pow, lerp, f20, fArr6, cbrt, (float) Math.pow(cbrt, 0.25d), sqrt);
    }
