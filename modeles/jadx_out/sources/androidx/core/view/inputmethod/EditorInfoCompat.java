...
public final class EditorInfoCompat {
    ...
    public static CharSequence getInitialTextAfterCursor(@NonNull EditorInfo editorInfo, int i, int i6) {
        ...
        int i10 = editorInfo.extras.getInt(CONTENT_SELECTION_END_KEY);
        int min = Math.min(i, charSequence.length() - i10);
        return (i6 & 1) != 0 ? charSequence.subSequence(i10, min + i10) : TextUtils.substring(charSequence, i10, min + i10);
    }
    ...
    private static void trimLongSurroundingText(EditorInfo editorInfo, CharSequence charSequence, int i, int i6) {
        ...
        int i12 = 2048 - i11;
        int min = Math.min(charSequence.length() - i6, i12 - Math.min(i, (int) (i12 * 0.8d)));
        int min2 = Math.min(i, i12 - min);
        int i13 = i - min2;
        ...
        setSurroundingText(editorInfo, i11 != i10 ? TextUtils.concat(charSequence.subSequence(i13, i13 + min2), charSequence.subSequence(i6, min + i6)) : charSequence.subSequence(i13, min2 + i11 + min + i13), min2, i11 + min2);
    }
}
