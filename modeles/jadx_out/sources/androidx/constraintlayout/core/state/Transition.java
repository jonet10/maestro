...
public class Transition implements TypedValues {
    ...
    public static class OnSwipe {
        ...
        public float getDestinationPosition(float f, float f10, float f11) {
            float abs = (((Math.abs(f10) * 0.5f) * f10) / this.mMaxAcceleration) + f;
    ...
    public float dragToProgress(float f, int i, int i6, float f10, float f11) {
        ...
        if (f16 != 0.0f) {
            ...
            abs = Math.abs(direction2[1]) * f11;
            f12 = fArr[1];
        }
        return (abs / f12) * this.mOnSwipe.getScale();
    }
    ...
    public void fillKeyPositions(WidgetFrame widgetFrame, float[] fArr, float[] fArr2, float[] fArr3) {
        ...
        for (int i6 = 0; i6 <= 100; i6++) {
            HashMap<String, KeyPosition> hashMap = this.mKeyPositions.get(Integer.valueOf(i6));
