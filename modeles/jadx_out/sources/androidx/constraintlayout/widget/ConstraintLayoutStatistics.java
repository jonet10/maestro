...
public class ConstraintLayoutStatistics {
    ...
    private String compare(DecimalFormat decimalFormat, ConstraintLayoutStatistics constraintLayoutStatistics, int i) {
        String n10 = x.n(x.u(fmt(decimalFormat, getValue(i) * 1.0E-6f, 7), " -> "), fmt(decimalFormat, constraintLayoutStatistics.getValue(i) * 1.0E-6f, 7), "ms");
        String n11 = x.n(new StringBuilder(), WORD_PAD, geName(i));
        return l.w("CL Perf: ", n11.substring(n11.length() - MAX_WORD).concat(" = "), n10);
    }

    private String fmt(DecimalFormat decimalFormat, float f, int i) {
        StringBuilder t9 = x.t(new String(new char[i]).replace((char) 0, ' '));
        t9.append(decimalFormat.format(f));
    ...
    private String compare(ConstraintLayoutStatistics constraintLayoutStatistics, int i) {
        ...
        String n10 = x.n(new StringBuilder(), WORD_PAD, geName(i));
        return l.w("CL Perf: ", n10.substring(n10.length() - MAX_WORD).concat(" = "), str);
    }

    private String log(DecimalFormat decimalFormat, int i) {
        String fmt = fmt(decimalFormat, getValue(i) * 1.0E-6f, 7);
        String n10 = x.n(new StringBuilder(), WORD_PAD, geName(i));
        return l.w("CL Perf: ", n10.substring(n10.length() - MAX_WORD).concat(" = "), fmt);
    }
    ...
    private String log(int i) {
        String l10 = Long.toString(getValue(i));
        String n10 = x.n(new StringBuilder(), WORD_PAD, geName(i));
        return l.w("CL Perf: ", n10.substring(n10.length() - MAX_WORD).concat(" = "), l10);
    }
}
