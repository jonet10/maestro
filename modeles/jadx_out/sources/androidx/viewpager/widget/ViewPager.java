...
public class ViewPager extends ViewGroup {
    ...
    private void calculatePageOffsets(ItemInfo itemInfo, int i, ItemInfo itemInfo2) {
            ...
            if (i11 < i12) {
                ...
                while (i13 <= itemInfo.position && i14 < this.mItems.size()) {
                    ...
                    while (i13 < itemInfo4.position) {
                        f10 += this.mAdapter.getPageWidth(i13) + f;
                        i13++;
                    }
                ...
                while (true) {
                    ...
                    while (i11 > itemInfo3.position) {
                        f11 -= this.mAdapter.getPageWidth(i11) + f;
                        i11--;
                    }
        ...
        int i16 = i15 - 1;
        this.mFirstOffset = i15 == 0 ? f12 : -3.4028235E38f;
        int i17 = count - 1;
        this.mLastOffset = i15 == i17 ? (itemInfo.widthFactor + f12) - 1.0f : Float.MAX_VALUE;
        int i18 = i - 1;
        while (i18 >= 0) {
            ...
            while (true) {
                ...
                f12 -= this.mAdapter.getPageWidth(i16) + f;
                i16--;
            }
            f12 -= itemInfo7.widthFactor + f;
            itemInfo7.offset = f12;
            if (i10 == 0) {
        ...
        while (i20 < size2) {
            ...
            while (true) {
                ...
                f13 += this.mAdapter.getPageWidth(i19) + f;
                i19++;
            }
            if (i6 == i17) {
                this.mLastOffset = (itemInfo8.widthFactor + f13) - 1.0f;
            }
            itemInfo8.offset = f13;
            f13 += itemInfo8.widthFactor + f;
            i20++;
            i19++;
        }
        this.mNeedCalculatePageOffsets = false;
    ...
    private boolean pageScrolled(int i) {
        ...
        float f10 = ((i / f) - infoForCurrentScrollPosition.offset) / (infoForCurrentScrollPosition.widthFactor + (i6 / f));
        this.mCalledSuper = false;
    ...
    private void scrollToItem(int i, boolean z9, int i6, boolean z10) {
        ...
        if (infoForPosition != null) {
            i10 = (int) (Math.max(this.mFirstOffset, Math.min(infoForPosition.offset, this.mLastOffset)) * getClientWidth());
        } else {
            i10 = 0;
        }
        if (z9) {
            smoothScrollTo(i10, 0, i6);
            if (z10) {
            ...
            if (itemPosition != -1) {
                if (itemPosition == -2) {
                    ...
                    if (i10 == itemInfo.position) {
                        i = Math.max(0, Math.min(i10, count - 1));
                    }
                } else {
    ...
    public boolean onTouchEvent(MotionEvent motionEvent) {
        ...
        if (action == 0) {
            ...
            if (action == 2) {
                if (!this.mIsBeingDragged) {
                    ...
                    if (findPointerIndex == -1) {
                        ...
                        if (abs > this.mTouchSlop && abs > abs2) {
                            ...
                            this.mLastMotionX = f10 > 0.0f ? f + i : f - i;
                            this.mLastMotionY = y10;
                            setScrollState(1);
                            setScrollingCacheEnabled(true);
                            ViewParent parent = getParent();
                ...
                if (this.mIsBeingDragged) {
                    z9 = performDrag(motionEvent.getX(motionEvent.findPointerIndex(this.mActivePointerId)));
                }
            } else if (action != 3) {
                if (action == 5) {
                    int actionIndex = motionEvent.getActionIndex();
    ...
    */
    ...
    public void smoothScrollTo(int i, int i6, int i10) {
        ...
        float f = clientWidth;
        float f10 = i14;
        float distanceInfluenceForSnapDuration = (distanceInfluenceForSnapDuration(Math.min(1.0f, (Math.abs(i12) * 1.0f) / f)) * f10) + f10;
        int abs = Math.abs(i10);
        int min = Math.min(abs > 0 ? Math.round(Math.abs(distanceInfluenceForSnapDuration / abs) * 1000.0f) * 4 : (int) (((Math.abs(i12) / ((this.mAdapter.getPageWidth(this.mCurItem) * f) + this.mPageMargin)) + 1.0f) * 100.0f), 600);
