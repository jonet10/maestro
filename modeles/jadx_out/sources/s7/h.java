package s7;

/* compiled from: r8-map-id-035a71e92ccd2a2b8039d43fa6fa76ac249b2a7f96061be914156707964ce49d */
/* loaded from: C:\Users\herod\Downloads\Jeux\classes3.dex */
public final class h implements t6.h {

    /* renamed from: a, reason: collision with root package name */
    public final /* synthetic */ t6.h f9366a;

    /* renamed from: b, reason: collision with root package name */
    public final Throwable f9367b;

    public h(Throwable th, t6.h hVar) {
        this.f9366a = hVar;
        this.f9367b = th;
    }

    @Override // t6.h
    public final Object fold(Object obj, d7.p pVar) {
        return this.f9366a.fold(obj, pVar);
    }

    @Override // t6.h
    public final t6.f get(t6.g gVar) {
        return this.f9366a.get(gVar);
    }

    @Override // t6.h
    public final t6.h minusKey(t6.g gVar) {
        return this.f9366a.minusKey(gVar);
    }

    @Override // t6.h
    public final t6.h plus(t6.h hVar) {
        return this.f9366a.plus(hVar);
    }
}
