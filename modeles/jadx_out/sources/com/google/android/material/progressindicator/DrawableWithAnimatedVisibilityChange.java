...
abstract class DrawableWithAnimatedVisibilityChange extends Drawable implements Animatable2Compat {
    ...
    public float getPhaseFraction() {
        ...
        if (this.baseSpec.hasWavyEffect(isDeterminateDrawable()) && this.baseSpec.waveSpeed != 0) {
            ...
            if (systemAnimatorDurationScale > 0.0f) {
                ...
                float uptimeMillis = (SystemClock.uptimeMillis() % r0) / ((int) ((((isDeterminateDrawable ? baseProgressIndicatorSpec.wavelengthDeterminate : baseProgressIndicatorSpec.wavelengthIndeterminate) * 1000.0f) / this.baseSpec.waveSpeed) * systemAnimatorDurationScale));
    ...
    @VisibleForTesting
    public void setMockHideAnimationRunning(boolean z9, @FloatRange(from = 0.0d, to = 1.0d) float f) {
        this.mockHideAnimationRunning = z9;
        this.mockGrowFraction = f;
    }

    @VisibleForTesting
    public void setMockPhaseFraction(@FloatRange(from = 0.0d, to = 1.0d) float f) {
        this.mockPhaseFraction = f;
    }

    @VisibleForTesting
    public void setMockShowAnimationRunning(boolean z9, @FloatRange(from = 0.0d, to = 1.0d) float f) {
